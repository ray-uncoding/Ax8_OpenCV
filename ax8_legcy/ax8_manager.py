...
class AX8Manager:
    def __init__(self, url, username, password):
        ...
        self.session = None
        self.running = False
        self.keep_alive_thread = None  # 新增心跳執行緒
        self.keep_alive_running = False  # 控制心跳執行緒的運行狀態

        # 使用相對路徑載入 YOLO 模型
        model_path = os.path.join(os.path.dirname(__file__), "../config/yolov8n.pt")
        try:
            self.model = YOLO(model_path)  # 確保路徑正確
    ...
    def login(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.start_keep_alive()  # 登錄成功後啟動心跳執行緒
                return True
            else:
